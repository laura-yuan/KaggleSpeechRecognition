...
class learning_rate_control():
    def __init__(self, max_step):
        ...
        self.learning_rate_current = 0.001
    ...
    def learning_rate_change(self, which_step):
        ...
        if (which_step >= 2 * self.smooth_n) & ( which_step%self.smooth_n == 0 ):
            ...
            if ((smoothed_triplet_loss_curr - smoothed_triplet_loss_prev) / smoothed_triplet_loss_prev) > -self.thresh:
                return self.learning_rate_current * 0.7
            else:
...
class jy_summary:
    ...
    def update(self, which_step, accuracy_value, entropy_value, confusion_matrix_value, triplet_loss_hard_value,
               triplet_loss_easy_value, learning_rate_value):
...
def main(_):
    ...
    model_settings = models.prepare_model_settings(
        ...
        FLAGS.batch_normalization_flag)
    ...
    logits, loss_triplet, dropout_prob, is_training_flag, trainable_encoding_var_list = models.create_verification_model(
        [fingerprint_anchor, fingerprint_positive, fingerprint_negative], model_settings)
    ...
    with tf.name_scope('train_encoding_layer_only'):
        train_step_encoding_only = tf.train.AdamOptimizer(
            learning_rate_input).minimize(loss_triplet, var_list=trainable_encoding_var_list)

    with tf.name_scope('train_encoding'):
        train_step_encoding = tf.train.AdamOptimizer(
            learning_rate_input).minimize(loss_triplet)
    ...
    for training_step in xrange(start_step, training_steps_max + 1):
        ...
        train_summary, train_accuracy, train_cross_entropy_value, train_confusion_matrix, predicted_value, _, _ = sess.run(
            [
                merged_summaries, evaluation_step, cross_entropy_mean, confusion_matrix, predicted_indices,
                train_step_conv,
        ...
        tf.logging.info(
            'Step #%d: rate %f, accuracy %.1f%%, cross entropy %f, triplet_loss_easy %0.1f, triplet_loss_hard %0.1f' %
            (training_step, learning_rate_value, train_accuracy * 100,
             train_cross_entropy_value, 0, 0))
    ...
    ## before testing, train the triplet loss.
    triplet_loss_training_steps_max = FLAGS.triplet_loss_training_steps_max
    # a small class to tune the learning rate adaptively.
    # you can test things
    learning_rate_triplet_loss_control = learning_rate_control(triplet_loss_training_steps_max)
    learning_rate_triplet_loss = learning_rate_triplet_loss_control.learning_rate_current

    train_summary_jy_triplet_loss = jy_summary(triplet_loss_training_steps_max)
    jy_summary_train_path_triplet_loss = os.path.join(FLAGS.summaries_dir, FLAGS.model_architecture + '_train_triplet_loss' + '.pickle')
    for training_step in xrange(1, triplet_loss_training_steps_max + 1):
        # Figure out what the current learning rate is.
        # use the triplot_loss to do it. PID controller.
        # Pull the audio samples we'll use for training.

        # minibatch. still use mini batch. do not train anything, just evaluate, and get the prediction.
        train_fingerprints, train_ground_truth, _ = audio_processor.get_data(
            FLAGS.batch_size, 0, model_settings, FLAGS.background_frequency,
            FLAGS.background_volume, time_shift_samples, 'training', sess)

        # Run the graph with this batch of training data.
        predicted_value = sess.run(predicted_indices,
            feed_dict={
                fingerprint_anchor: train_fingerprints,
                ground_truth_input: train_ground_truth,
                learning_rate_input: 0.1,
                dropout_prob: 1,
                is_training_flag: False
            })
        # given the predicted value, organize the triplet loss pairs.
        easy_triplets = input_data.verification_utils_prepare_triplet(train_ground_truth,
                                                                      label_count=model_settings['label_count'],
                                                                      hard_mode=False,
                                                                      num_of_triplets=FLAGS.batch_size)
        hard_triplets = input_data.verification_utils_prepare_triplet(train_ground_truth,
                                                                      label_count=model_settings['label_count'],
                                                                      hard_mode=True,
                                                                      num_of_triplets=FLAGS.batch_size,
        ...
        encoding_only_feed_dict = {fingerprint_anchor  : train_fingerprints[easy_triplets[:, 0]],
                                   ...
                                   learning_rate_input : learning_rate_triplet_loss,
                                   dropout_prob        : 1,
                                   is_training_flag    : False
                                   }
        triplet_loss_easy_value, _ = sess.run([loss_triplet, train_step_encoding_only], encoding_only_feed_dict)
        ...
        triplet_loss_hard_value, _ = sess.run([loss_triplet, train_step_encoding_only], encoding_feed_dict)

        train_summary_jy_triplet_loss.update(training_step - 1, 0,0,0,triplet_loss_hard_value, triplet_loss_easy_value, learning_rate_triplet_loss)

        learning_rate_triplet_loss = learning_rate_triplet_loss_control.update_value(training_step - 1, triplet_loss_easy_value)
        tf.logging.info(
            'Step #%d: rate %f, triplet_loss_easy %0.1f, triplet_loss_hard %0.1f, learning rate %f' %
            (training_step, learning_rate_triplet_loss, triplet_loss_easy_value, triplet_loss_hard_value, learning_rate_triplet_loss))
        if (training_step % 100 == 0 or
                training_step ==  triplet_loss_training_steps_max):
            checkpoint_path = os.path.join(FLAGS.train_dir,
                                           FLAGS.model_architecture + 'encoding_layer'+'.ckpt')
            tf.logging.info('Saving to "%s-%d"', checkpoint_path, training_step)
            saver.save(sess, checkpoint_path, global_step=training_step)
            # Save the jy_summary as well.
            train_summary_jy_triplet_loss.save(jy_summary_train_path_triplet_loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--triplet_loss_training_steps_max',
        type=int,
        default=1000,
        help= 'training step for the triplet loss'
    )
